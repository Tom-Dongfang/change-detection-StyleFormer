...
from ops.modules import MSASelfAttn
from timm.models.layers import DropPath
import torch.utils.checkpoint as cp
from models.sync_batchnorm import SynchronizedBatchNorm2d
from models.style_repin import StyleRepresentation
...
class MultiscaleExtractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        self.attn = MSASelfAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads,
...
class CTIBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W):
        ...
        for idx, blk in enumerate(blocks):  # vit
            x = blk(x, H, W)
...
class CNN_Style(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384):
        super().__init__()
        self.stem = nn.Sequential(*[
            nn.Conv2d(3, inplanes, kernel_size=3, stride=2, padding=1, bias=False),
            SynchronizedBatchNorm2d(inplanes, momentum=bn_mom),
        ...
        self.conv2 = nn.Sequential(*[
            ...
            SynchronizedBatchNorm2d(2 * inplanes, momentum=bn_mom),
            nn.ReLU(inplace=True)
        ])
        self.conv3 = nn.Sequential(*[
            nn.Conv2d(2 * inplanes, 4 * inplanes, kernel_size=3, stride=2, padding=1, bias=False),
            SynchronizedBatchNorm2d(4 * inplanes, momentum=bn_mom),
            nn.ReLU(inplace=True)
        ])
        ...
        self.StyleRep1 = StyleRepresentation(num_prototype=2, channel_size=inplanes, gamma=0.9)
        self.StyleRep2 = StyleRepresentation(num_prototype=2, channel_size=2 * inplanes, gamma=0.9)
    ...
    def forward(self, x):
        c1 = self.stem(x)
        c1 = self.StyleRep1(c1)

        c2 = self.conv2(c1)
        c2 = self.StyleRep2(c2)
